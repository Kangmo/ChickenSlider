...
-(void) parsePlistFile:(NSString*)filename
{
	...
	if(data && [data objectForKey:@"frames"])
	{
		...
		CCSpriteBatchNode *mgr = [CCSpriteBatchNode batchNodeWithFile:textureName capacity:1];
	...
	for (CXMLElement * curLayer in layers) 
	{
		...
		if([curLayer attributeForName:@"isTexture"])
		{
			if(![[managers allKeys] containsObject:[[curLayer attributeForName:@"id"] stringValue]])
			{
				...
				CCSpriteBatchNode *mgr = [CCSpriteBatchNode batchNodeWithFile:[[curLayer attributeForName:@"id"] stringValue] capacity:1];
...
-(void) clearSprites
{
	...
	{
		CCSpriteBatchNode *mgr = (CCSpriteBatchNode *)[managers objectForKey:key];
	...
	for (NSString* curKey in [spriteDescriptions allKeys]) 
	{
		...
		if([parts count]>1)
		{
			...
			if([spName isEqualToString:name])
			{
				...
				CCSprite * s = [CCSprite spriteWithBatchNode:[managers objectForKey:c.textureName] rect:c.rect];
				[sprites addObject:[s retain]];
				CCSpriteBatchNode *mgr = (CCSpriteBatchNode *)[managers objectForKey:c.textureName];
				[mgr addChild:s];
...
{
	CCSprite * s = [self getSpriteWithName:name fromTexture:texName];
	if(s)
	{
