...
@synthesize delegate;
@synthesize commDelegate;
@synthesize isGameCenterAvailable;
@synthesize lastError;
@synthesize achievements;
@synthesize currentMatch;
@synthesize currentMatchPlayers;
...
-(void) dealloc
{
    ...
    // BUGBUG : should we call [currentMatch disconnect]?
	[currentMatch release];
    self.currentMatchPlayers = nil;
...
- (void) loadPlayerData: (NSArray *) identifiers
{
    [GKPlayer loadPlayersForIdentifiers:identifiers withCompletionHandler:^(NSArray *players, NSError *error) {
        ...
        if (players != nil)
        {
            self.currentMatchPlayers = [players retain];// Process the array of GKPlayer objects.
        }
    }];
}

-(GKPlayer *) getPlayerByID:(NSString*)playerID {
    if (isGameCenterAvailable == NO)
		return nil;
    
    for (GKPlayer * player in self.currentMatchPlayers) {
...
-(void) initMatchInvitationHandler
{
	...
	[GKMatchmaker sharedMatchmaker].inviteHandler = ^(GKInvite* acceptedInvite, NSArray* playersToInvite)
	{
		...
		{
            // BUGBUG : understand how users can invite players in game center UI.
...
{
	[commDelegate onReceivedData:data fromPlayer:playerID];
}
